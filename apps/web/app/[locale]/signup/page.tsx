...
export default async function SignupPage() {
  ...
  return (
    <main className="signup-reveal relative min-h-[calc(100vh-5rem)] w-full max-w-full overflow-x-hidden overflow-y-hidden bg-[var(--auth-bg)] pt-0 text-[var(--auth-primary)]">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_22%,rgb(var(--auth-divider-rgb)/0.08),transparent_24%),radial-gradient(circle_at_78%_66%,rgb(var(--auth-divider-rgb)/0.05),transparent_22%)]" />

      <div className="relative mx-auto grid w-full max-w-[1050px] grid-cols-1 items-center gap-12 overflow-hidden px-6 md:min-h-[calc(100vh-5rem-6rem)] md:grid-cols-2 md:gap-14 lg:gap-16">
        <div className="relative flex items-start justify-center py-6 md:py-10">
          ...
          <div className="relative z-10 flex w-full max-w-[30rem] flex-col items-start">
            ...
            <div className="relative mt-5 flex h-[122px] w-[122px] items-center justify-center self-center md:mt-6 md:h-[148px] md:w-[148px]">
              ...
              <div className="absolute inset-x-5 bottom-3 h-6 rounded-full bg-[radial-gradient(circle,var(--signup-cube-ground),transparent)] blur-2xl" />
            </div>
          </div>
        </div>

        <div className="pointer-events-none absolute left-1/2 top-1/2 hidden -translate-x-1/2 -translate-y-1/2 md:block">
          <div className="h-[300px] w-px bg-[linear-gradient(to_bottom,transparent,rgb(var(--auth-divider-rgb)/0.2),transparent)]" />
        </div>

        <div className="relative flex items-start justify-start py-6 md:py-12">
          <div className="w-full max-w-[26rem]">
            ...
            <form className="mt-5 space-y-4.5">
              <div className="grid gap-3 md:grid-cols-2">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Barcelona operator"
                  />
                </div>
              ...
              <button
                type="submit"
                className="flex h-11 w-full items-center justify-center rounded-xl border border-[rgb(var(--auth-input-border-rgb)/0.26)] px-4 text-[14px] font-semibold text-[var(--auth-main-btn-fg)] transition hover:scale-[1.02] active:scale-[0.99]"
                style={{
              ...
              <button
                type="button"
                className="flex h-10 w-full items-center justify-center gap-2.5 rounded-xl border border-[rgb(var(--auth-input-border-rgb)/0.22)] bg-[rgb(var(--auth-google-bg-rgb)/var(--auth-google-bg-alpha))] px-4 text-[13px] font-medium text-[var(--auth-google-text)] shadow-[inset_0_1px_0_rgba(255,255,255,0.12),0_10px_24px_rgba(0,0,0,0.22)] backdrop-blur-[12px] transition hover:-translate-y-0.5 hover:bg-[rgb(var(--auth-google-bg-rgb)/0.05)]"
              >
                <span className="flex h-5 w-5 items-center justify-center rounded-full border border-[rgb(var(--auth-input-border-rgb)/0.18)] bg-[radial-gradient(circle_at_30%_30%,rgba(255,255,255,0.22),rgba(165,162,131,0.12))] text-[9px] font-semibold text-white shadow-[0_4px_12px_rgba(0,0,0,0.2)]">
                  G
                </span>
            ...
            <div className="mt-5 text-[13px] text-[var(--auth-secondary)]">
              {t("switchToLogin")}{" "}
              <Link href="/login" className="text-[var(--auth-primary)] transition hover:text-white">
                Sign In
              </Link>
            </div>
          </div>
